...
void greeting() { //greeting the user
    printf("********************************************************\n");
...
int playRound(int round) {
    printf("Welcome to a new round %d!\n", round); //letting user know there is a new round
    ...
    while (toothpicks != 0) {
       ...
       int humanPick() {
            ...
            scanf("%d", &pick);
            if (pick==1||pick==2||pick==3||pick<toothpicks) //not printing if user's input is incorrect
                printf("Ok... you took %d off the table.\n",pick);
                return pick;
        } 
        int userPick = humanPick();
        while (userPick < 1 || userPick > 3 || userPick > toothpicks) { //checking if user's input is between 1-3
            printf("Oh no no, you can't do that in this game! Try again.\n"); //printing message if input is incorrect
            userPick = humanPick();
        }
        ...
        int computerPick(int choice, int leftover) { // if added to check 3 different outcomes from the computer
            int total;
            if (leftover > 4) {
                total = 4 - choice; 
            } 
        ...
        count = 1; //computer's turn
    	
    	if (toothpicks == 0) 
        	return count;
    }
		
    return count; //last person who picked turn
}
...
void winnerAnnouncment(int user) {
    if (user == 0) { //printing messages if user / computer win
